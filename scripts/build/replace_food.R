...
replace_food_w_better <- function(orig_menu, df = abbrev, max_offender, nutrient_to_restrict, cutoff = 0.5,
                                  verbose = TRUE) {
  scaled <- scaled %>% 
  ...
  if ("shorter_desc" %in% names(orig_menu)) {
    replacment_food_pool <- replacment_food_pool %>% do_menu_mutates()
  }
