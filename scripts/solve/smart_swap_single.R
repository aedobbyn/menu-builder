...
smart_swap_single <- function(orig_menu, cutoff = 0.5, silent = FALSE) {
    ...
    for (m in seq_along(mr_df$must_restrict)) {    # for each row in the df of must_restricts
      ...
      if (silent == FALSE) { message(paste0("------- The nutrient we're restricting is ", nut_to_restrict, ". It has to be below ", mr_df$value[m])) }
      ...
      if (silent == FALSE) { message(paste0("The original total value of that nutrient in our menu is ", to_restrict)) }
      ...
      if (to_restrict > mr_df$value[m]) {     # if the amount of the must restrict in our current menu is above the max value it should be according to mr_df
        ...
        if (silent == FALSE) { message(paste0("The worst offender in this respect is ", orig_menu[max_offender, ]$Shrt_Desc)) }
        ...
        if (silent == FALSE) { message(paste0("Our new value of this must restrict is ", to_restrict)) }
      } else {
        if (silent == FALSE) { message("We're all good on this nutrient.") }
      }
    }
