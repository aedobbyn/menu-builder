...
`read_html()` from the `xml2` package will return us the raw HTML for a given page. We're only interested in the recipe portion of that, so using the Chrome inspector or the [SelectorGadget Chrome extension](http://selectorgadget.com/) we can figure out what the CSS tag is of the content itself. That happens to be the class `.checkList__line`. So, we'll pluck everything tagged with `.checkList__line` using `html_nodes()` and return text we can use with `html_text()`.

Next step is to pare out all the stray garbage left over like `\n` new lines etc., and return our recipe.
...
Cool, so we've got two functions now, one for reading the content from a URL and one for grabbing the part of it that we want and cleaning it up.
