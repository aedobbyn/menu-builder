...
The USDA does have an open API so you can create an API key and grab foods from them with requests along the lines of the below. The [API documentation](https://ndb.nal.usda.gov/ndb/doc/apilist/API-FOOD-REPORTV2.md) walks through the format for requesting data. 
...
We've specified just 4 nutrients we want here
...
Before unnesting our `nutrients` list column, we'll want to take `gm` and `value` from character
...
Missing values are coded as `--` so we'll change those into `NA`s.
