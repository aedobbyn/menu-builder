...
But, as it happens, protion sizes don't usually consist of just one number. There are a few hurdles: 1) complex fractions, 2) multiple items of the same item, and 3) ranges. A complex fraction would be something a recipe item like "2 1/3 cups of flour." A multiple of an item is, e.g., "4 12oz bottles of beer." A range would be "6-7 tomatoes." 


Here is a fake recipe to illustrate some of those cases:
```{r some_recipes_tester}
some_recipes_tester
```


Rather than start doing conditional random field-level smart to get around these problems, to start off I started writing a few rules of thumb.

**Complex fractions**

First, a way to turn a single fraction into a decimal form. We keep them `as.character`
...
And finally, if we neither need to get a range nor multiply/add portions, we'll just take whatever the first number is in there.
