...
Let's do that for a spectrum of minimum portion sizes with a fixed max number of swaps we're willing to do. Like we did before, we split the range `from:to` into `n_intervals` and do `n_sims` at each interval, recording the number of  swaps it took to solve it. Our return tibble this time includes the swap status
