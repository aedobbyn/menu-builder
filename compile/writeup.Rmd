...
```{r all_units_df}
all_units <- c(ingredient_units, abbrev_dict$name, abbrev_dict$key, "inch")
...
Let's see if we can assign food groups to ingredients in our recipe.
...
butter_req <- 
  paste0("https://api.nal.usda.gov/ndb/search/?format=json&q=", query_term, "&sort=n&max=25&offset=0&api_key=", key)

butter_res <- butter_req %>% jsonlite::fromJSON()
```
